...
public class DebtCustomerDetailDialog extends JDialog {
    ...
    private final JPanel debtAmountRow;
    ...
    public DebtCustomerDetailDialog(Window owner, int debtId, String nickname, double debtAmount,
        ...
        debtAmountField = valueField(PesoFormat.formatAmountOnly(debtAmount), 14, Font.BOLD);
        JLabel pesoLabel = new JLabel(PesoFormat.SIGN);
        pesoLabel.setFont(PesoFormat.font(Font.BOLD, 14));
        pesoLabel.setForeground(TEXT_DARK);
        debtAmountRow = new JPanel(new BorderLayout(4, 0));
        debtAmountRow.setOpaque(false);
        debtAmountRow.add(pesoLabel, BorderLayout.WEST);
        debtAmountRow.add(debtAmountField, BorderLayout.CENTER);
    ...
    private JPanel buildDetailsCard() {
        ...
        gbc.gridx = 1;
        card.add(detailCell(ProfileIcons.field(34, ProfileIcons.Field.WALLET), "Outstanding Debt", debtAmountRow), gbc);
    ...
    private JPanel detailCell(ImageIcon icon, String label, JComponent value) {
        ...
        } else if (value instanceof JPanel panel && panel.getComponentCount() > 0
    ...
    private JTextField valueField(String text, int size, int style) {
        ...
        field.setFont(PesoFormat.font(style, size));
    ...
    private static double parseAmount(String raw) {
        try {
            return Double.parseDouble(PesoFormat.stripForParse(raw));
